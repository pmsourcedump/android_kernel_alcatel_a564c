...
#include <linux/reboot.h>//add by jch for imei check disable watch dog
...
//add by jch for check imei number 
#define REBOOT_TIME 10
#define REBOOT_LONG_TIME 30
#define SECURITY_CHECK_MAGIC "Security Error Tampered: Error Code "
#define MEID_WARNING "Security Error Code 797:\n\nThis phone has been flashed with unauthorized software. Restarting in 10 seconds. Call your mobile operator for additional support.\n"
#define SPC_WARNING "Security Error Code 55:\n\nThis phone has been flashed with unauthorized software. Restarting in 10 seconds. Call your mobile operator for additional support.\n"
#define IMEI_WARNING "Security Error:\n\nThis phone has been flashed with unauthorized software & is locked. Call your mobile operator for additional support.  Please note that repair/return for this issue may have additional cost.\n"
#define UNKNOWN_WARNING "Security Error Unknown:\n\nThis phone has been flashed with unauthorized software & is locked. Call your mobile operator for additional support.  Please note that repair/return for this issue may have additional cost.\n"
#define MEID_ERROR "1"
#define SPC_ERROR "2"
#define IMEI_ERROR "3"
#define SPC_OUT_ERROR "4" //add by xiaoyong.wu for spc attempts out limits
#define NO_SEC_ERRO 0
#define MEID_RESP 1
#define SPC_RESP 2
#define IMEI_RESP 3
#define SPC_OUT_COUNT 4 //add by xiaoyong.wu for spc attempts out limits

#define UNKNOWN_RESP 9
extern void fbcon_check_word(char *str);
extern void fbcon_putc(char c);
extern void fbcon_show(void );
extern void fbcon_clear(void);
extern void pet_watchdog_ext(void);
extern void set_spinlock_delay_times(void);
...
int lcd_show_text(const char *fmt, ...)
{
	char buf[256];
	int err;
	va_list ap;
...
static int check_imei_form_msg(void)
{
       u32 size;
	...
	if (!smem_reason || !size) {
		pr_info("JCH:check_imei_form_msg, smem_get_entry_no_rlock failed.\n");
		return NO_SEC_ERRO;
	}
	if (!smem_reason[0]) {
		pr_info("JCH:check_imei_form_msg: (unknown, empty string found).\n");
		return NO_SEC_ERRO;
	}
	strlcpy(reason, smem_reason, min(size, sizeof(reason)));
	pr_info("JCH:modem subsystem notice reason: %s.\n", reason);
	smem_reason[0] = '\0';
	wmb();
	msg_check=strstr(reason,SECURITY_CHECK_MAGIC);
       if (!msg_check) {
		return NO_SEC_ERRO;
	}
	pr_info("JCH:Modem subsystem notice us Security information have been changed!.\n");
	pr_info("JCH:Security check error code: %s.\n", msg_check+strlen(SECURITY_CHECK_MAGIC));
       if(!strncmp((msg_check+strlen(SECURITY_CHECK_MAGIC)),MEID_ERROR,1)){
                  pr_info("JCH: MEID have been changed!.\n");
		    return MEID_RESP;
	 }
	 else if(!strncmp((msg_check+strlen(SECURITY_CHECK_MAGIC)),SPC_ERROR,1))
	 {
                  pr_info("JCH: SPC have been changed!.\n");
		    return SPC_RESP;
	 }
	 else if(!strncmp((msg_check+strlen(SECURITY_CHECK_MAGIC)),IMEI_ERROR,1))
	 {
                  pr_info("JCH: IMEI have been changed!.\n");
		    return IMEI_RESP;
	 }
	 //add by xiaoyong.wu for spc attempts out limits
	 else if(!strncmp((msg_check+strlen(SECURITY_CHECK_MAGIC)),SPC_OUT_ERROR,1))
	 {
                  pr_info("WXY: SPC check attempts is out  the limit ! do restart.\n");
		    return SPC_OUT_COUNT;
	 }
	 else
	 {
                  pr_info("JCH: Security info(Unknown reason) have been changed!.\n");
		    return UNKNOWN_RESP;
	 }
	 return NO_SEC_ERRO;
}
//end add by jch for check imei number
...
{
       int i;//add by jch for imei number check
	struct modem_data *drv = subsys_to_drv(dev_id);

	/* Ignore if we're the one that set the force stop GPIO */
	if (drv->crash_shutdown)
		return IRQ_HANDLED;
//add by jch for imei number check       
       switch (check_imei_form_msg()){
	   	
		case MEID_RESP:			
                     lcd_show_text(MEID_WARNING);
		       mdelay(200);
			pr_info("JCH: Rebooting in %d second ...\n",REBOOT_TIME);
			set_spinlock_delay_times();
                     for (i = 0; i < REBOOT_TIME* 1000; i += 100) {
				 lcd_show_text(MEID_WARNING);
				 pet_watchdog_ext();						
			        mdelay(100);
		       }
			pr_info("JCH: Rebooting now ...\n");			
			machine_restart(NULL);
			break;

		case SPC_RESP:			
                     lcd_show_text(SPC_WARNING);
		       mdelay(200);
			pr_info("JCH: Rebooting in %d second ...\n",REBOOT_TIME);
			set_spinlock_delay_times();
                     for (i = 0; i < REBOOT_TIME* 1000; i += 100) {
		...
		case IMEI_RESP:			
                     lcd_show_text(IMEI_WARNING);
		       mdelay(200);
	   	       pr_info("JCH: Phone will show warning msg forever !!!\n");	
			set_spinlock_delay_times();
			while (1) {
				 lcd_show_text(IMEI_WARNING);
				 pet_watchdog_ext();		 
		       ...
		       //mdelay(REBOOT_LONG_TIME*1000);
			pr_info("JCH: Rebooting now ...\n");
			machine_restart(NULL);
			break;
		case SPC_OUT_COUNT: //add by xiaoyong.wu for spc attempts out limits
			pr_info("WXY: Rebooting now ...\n");
			machine_restart(NULL);
			break;

		case UNKNOWN_RESP:			
			lcd_show_text(UNKNOWN_WARNING);
		      	mdelay(200);	             
	   	       pr_info("JCH: Phone will show warning msg forever !!!\n");
			set_spinlock_delay_times();  
			while (1) {
...
//end add by jch for imei number check 
