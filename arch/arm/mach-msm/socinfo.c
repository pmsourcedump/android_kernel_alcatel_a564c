...
//add by zcl for hardware_board_id
uint32_t socinfo_get_hw_version(void)
{
	...
	if (board_id ==0xFFFFFFFF)
	{
		...
		//pr_info("kernel::::%s,line:%d,board_id_smem:%d",__func__,__LINE__,*board_id_smem);
		if(board_id_smem!=NULL){
			board_id =*board_id_smem; 
			pr_info("kernel::::%s,line:%d,board_id_smem:%d",__func__,__LINE__,board_id);
			}
		else
		{
			pr_info( "NULL pointer of board_id_smem" );
			return -1;
		}
	}
...
//add by zcl for GPIO_BOARD_ID	
static uint32_t socinfo_gpio_board_id(void)   
{   
	uint32_t * gpio_board_id_smem; 
	uint32_t test_forgpio = 0xFFFFFFFF;
	gpio_board_id_smem = smem_alloc(SMEM_VERSION_GPIO_BOARD_ID, sizeof(uint32_t));
	if(gpio_board_id_smem!=NULL)
	{
	pr_info("%s:zcl:kernel:share memery get:%u\n", __func__,*gpio_board_id_smem);
	test_forgpio = *gpio_board_id_smem;
	pr_info("%s:zcl: kernel!get:%u\n", __func__,test_forgpio);
	}
	return  test_forgpio;
}
//end add by zcl for GPIO_BOARD_ID
...
//add by xiaoyong.wu for efuse check
static ssize_t
		     ...
		     char *buf)
{
	...
	efuse = smem_alloc(SMEM_VERSION_OEM_EFUSE, 2*sizeof(uint32_t));
	if(efuse != NULL){
		pr_info("[wxy]----efuse = 0x%08x     oem_modem id = 0x%08x\n",*efuse,*(efuse+1));
		if(*efuse == 0x00303030)
			return snprintf(buf, PAGE_SIZE, "efuse: 0x%04x 0x%04x ",*(efuse+1)&0xffff,(*(efuse+1))>>16);
		else
			return snprintf(buf, PAGE_SIZE, "non-efuse");
		}
...
msm_get_hw_version(struct device *dev,
		     ...
		     char *buf)
{
	pr_info("kernel::::%s,line:%d,msm_get_hw_version",__func__,__LINE__);
	pr_info("kernel::::%s,line:%d,socinfo_get_hw_version:%u",__func__,__LINE__,socinfo_get_hw_version());
	return snprintf(buf, PAGE_SIZE, "%u\n",
		socinfo_get_hw_version());
}

//end add by zcl for hardware_board_id
...
msm_get_gpio_board_id(struct device *dev,
			...
			char *buf)	
{	
	pr_info("%s:zcl: msm_get_gpio_board_id = %u \n", __func__, socinfo_gpio_board_id());
...
//add by xiaoyong.wu for oem check
...
static struct device_attribute msm_soc_attr_msm_get_gpio_board_id =	
	__ATTR(gpio_for_board_id, S_IRWXUGO, msm_get_gpio_board_id,  NULL);
   //end add by zcl for GPIO_BOARD_ID
...
static void __init populate_soc_sysfs_files(struct device *msm_soc_device)
{
	...
	device_create_file(msm_soc_device,&msm_soc_attr_oem_efuse); // add by xiaoyong.wu for efuse check
	//add by zcl for hardware_board_id
	device_create_file(msm_soc_device,&msm_soc_attr_hw_version);
	pr_info("%s:msm_soc_attr_hw_version\n", __func__);
    //end add by zcl for hardware_board_id
	//add by zcl for GPIO_BOARD_ID	
	device_create_file(msm_soc_device,&msm_soc_attr_msm_get_gpio_board_id);
	pr_info("%s:msm_soc_attr_msm_get_gpio_board_id\n", __func__);
	//end add by zcl for GPIO_BOARD_ID
...
static int __init socinfo_init_sysdev(void)
{
	...
	//add by zcl for GPIO BOARD ID
	bod_id = socinfo_gpio_board_id();
	//end by zcl for GPIO BOARD ID
