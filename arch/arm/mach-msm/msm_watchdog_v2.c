 ...
 */
#define CONFIG_MSM_WATCHDOG_CTX_PRINT 1//add by jch for watch dog ramdump
...
#endif
//end add by jch
#include <linux/lcd_fbcon.h>//add by jch for imei check disable watch dog

struct msm_watchdog_data *wdog_dd_ext;//add by jch for imei check disable watch dog
...
static struct msm_watchdog_data *g_wdog_dd;  //add by chenghui.jia for test watchdog dump
...
//add by jch for imei check disable watch dog
...
//end by jch for imei check disable watch dog
...
#define TZBSP_DUMP_CTX_VERSION		4//modify by jch from 2 to 4 for watchdog tz ramdump
...
static void msm_wdt_ctx_print(struct msm_wdt_ctx *ctx)
{
	...
	bi_powerup_reason_wdog();
	MSMWDTD("\n*** Might be Watchdog Bite ***\n");
	MSMWDTD("\n*** Dump panic info ***\n");//add panic string in tz dump by jch 
	MSMWDTD("sc_status: ");
	for (i = 0; i < cpu_count; i++)
...
static void msm_watchdog_alloc_buf(struct msm_watchdog_data *wdog_dd)
{
	...
	struct msm_wdt_ctx *ctx;
      // phys_addr_t base = 0x19080000;
	//phys_addr_t size_phy = MSM_WDT_CTX_SIZE;
	...
	//if(memblock_reserve(base,size_phy)==0)
	  //   phys = (unsigned long)base;	
	if (!phys) {
	...
	wdog_dd->scm_regsave_size = MSM_WDT_CTX_SIZE;
       pr_info("JCH:watchdog wdog_dd->scm_regsave ==0x%08lX,wdog_dd->scm_regsave_size=0x%08lX.\n",wdog_dd->scm_regsave,wdog_dd->scm_regsave_size );	
	dev_dbg(wdog_dd->dev, "buf 0x%08lX size 0x%X (reserved 0x%X)\n",
			phys, MSM_WDT_CTX_SIZE, size);
	ctx = ioremap(phys, MSM_WDT_CTX_SIZE);
	//ctx=(struct msm_wdt_ctx *)phys;
	if (!ctx) {
...
static int __devinit msm_watchdog_probe(struct platform_device *pdev)
{
	...
	gdata = *wdog_dd;//add by jch for imei check disable watch dog
	if (ret)
		goto err;
	...
	g_wdog_dd = wdog_dd;//add by jch for imei check disable watch dog
	queue_work_on(0, wdog_wq, &wdog_dd->init_dogwork_struct);
	wdog_dd_ext = wdog_dd;//add by jch for imei check disable watch dog
	return 0;
err:
	destroy_workqueue(wdog_wq);
	kzfree(wdog_dd);
...
{
    struct wdog_disable_work_data work_data;
    struct msm_watchdog_data *wdog_dd =wdog_dd_ext;
    mutex_lock(&wdog_dd->disable_lock);
    if (enable) {
		pr_info("JCH:Now go to disable watch dog!");
		work_data.wdog_dd = wdog_dd;
		init_completion(&work_data.complete);
		INIT_WORK_ONSTACK(&work_data.work, wdog_disable_work);
		queue_work_on(0, wdog_wq, &work_data.work);
...
{
       pet_watchdog(wdog_dd_ext);

}
//end add by jch for imei check disable watch dog
