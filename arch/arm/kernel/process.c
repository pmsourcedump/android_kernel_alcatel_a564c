...
static void show_data(unsigned long addr, int nbytes, const char *name)
{
	...
	for (i = 0; i < nlines; i++) {
		...
		for (j = 0; j < 8; j++) {
			u32	data;
			/*
                          * vmalloc addresses may point to memory-mapped peripherals.
                          * wenming adds 'is_vmalloc_addr(p)' for bug709008 20140618
                        */			
                        if (is_vmalloc_addr(p) || probe_kernel_address(p, data)) {
				printk(" ********");
			} else {
				printk(" %08x", data);
			}
