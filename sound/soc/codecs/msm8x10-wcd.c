...
//ting.kang add for headset detecte error
static int on_demand_regulator_control(struct on_demand_supply *supply, bool enable, u8 shift);
...
//add by JRD_SZ(ting.kang@tcl.com), HAC,12/30/2013, begin
...
//add by JRD_SZ(ting.kang@tcl.com), HAC,12/30/2013, end
...
//ting.kang add for headset detecte error start
static int on_demand_regulator_control(struct on_demand_supply *supply, bool enable, u8 shift)
{
	int ret = 0;
...
//ting.kang add for headset detecte error end
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		ret = on_demand_regulator_control(supply, true, w->shift);
		break;
	case SND_SOC_DAPM_POST_PMD:
		ret = on_demand_regulator_control(supply, false, w->shift);
		break;
	//ting.kang add for headset detecte error end
	default:
		break;
...
//ting.kang add for headset detecte error start
static int msm8x10_wcd_enable_ext_mb_source(struct snd_soc_codec *codec, bool turn_on, bool use_dapm)
{
	int ret = 0;
	 ...
	 bool enable)
{
	int rc;
	struct msm8x10_wcd_priv *msm8x10_wcd = snd_soc_codec_get_drvdata(codec);
    printk("msm8x10_wcd_enable_mbhc_micbias()");
	if (enable)
	...
	struct msm8x10_wcd_pdata *pdata)
{
	...
	//ting.kang modify the micbias capless mode to cap mode
	snd_soc_update_bits(codec, MSM8X10_WCD_A_MICB_1_CTL, 0x00,
			...
			const struct i2c_device_id *id)
{
    ...
    msm8x10_ear_power_hac_init(&client->dev); //add by JRD_SZ(ting.kang@tcl.com), HAC,12/30/2013
