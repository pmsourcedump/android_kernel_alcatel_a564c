...
#define WCD9XXX_V_CS_HS_MAX 500  //ting.kang modify for headset detect error
#define WCD9XXX_V_CS_NO_MIC 8 //5  ting.kang modify for headset detect error
					   ...
					   bool is_cs_enable)
{
	...
	if (mbhc->mbhc_cb && mbhc->mbhc_cb->enable_mb_source)
		mbhc->mbhc_cb->enable_mb_source(codec, true, true);  //ting.kang modify for headset detect error
...
static void wcd9xxx_cleanup_hs_polling(struct wcd9xxx_mbhc *mbhc)
{
	...
	if (mbhc->mbhc_cb && mbhc->mbhc_cb->enable_mb_source)
		mbhc->mbhc_cb->enable_mb_source(mbhc->codec, false, true);  //ting.kang modify for headset detect error
			  ...
			  unsigned long event_state)
{
	...
	for (i = 0, d = dt, ch = 0; i < sz; i++, d++) {
		...
		//ting.kang move for headset detect start
		/*if ((!d->mic_bias &&
			...
			printk("%s: within invalid range\n", __func__);
			type = PLUG_TYPE_INVALID;
			goto exit;
		}*/
		//ting.kang move for headset detect end
	}
...
//ting.kang modify for headset detect error start
	...
	//	   (dvddio->_vdces > minv + WCD9XXX_THRESHOLD_MIC_THRESHOLD))) {
			pr_debug("%s: Headset with threshold on MIC detected\n",
				 __func__);
//ting.kang modify for headset detect error end
			if (mbhc->mbhc_cfg->micbias_enable_flags &
			    (1 << MBHC_MICBIAS_ENABLE_THRESHOLD_HEADSET))
				mbhc->micbias_enable = true;
		} else {
...
static void wcd9xxx_swch_irq_handler(struct wcd9xxx_mbhc *mbhc)
{
	...
	if(insert) {
		//headset_insert_state = 1;
		adb_state = 2;
	}
	else {
		//headset_insert_state = 0;
		adb_state =3;
	}
	...
	if (mbhc->buttons_pressed & WCD9XXX_JACK_BUTTON_MASK) {
		...
		if (ret == 0) {
			...
			if (timecount >= 30)  {
				...
				#if defined(CONFIG_ADB_FORCE_OPEN)
				adb_state = 1; 
				#endif
			}
			presstime=0;
...
static void wcd9xxx_mbhc_cal(struct wcd9xxx_mbhc *mbhc)
{
	 ...
	 */
	if (mbhc->mbhc_cb && mbhc->mbhc_cb->enable_mb_source)
		mbhc->mbhc_cb->enable_mb_source(codec, true, false); //ting.kang modify for headset detect error
	...
	if (mbhc->mbhc_cb && mbhc->mbhc_cb->enable_mb_source)
		mbhc->mbhc_cb->enable_mb_source(codec, false, false);  //ting.kang modify for headset detect error
