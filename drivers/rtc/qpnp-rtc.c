...
qpnp_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alarm)
{
	...
	// add by xcb
	printk("qpnp_rtc_read_time: \n");
	printk("h:r:s=%d:%d:%d, d/m/y=%d/%d/%d\n",
...
static int __devinit qpnp_rtc_probe(struct spmi_device *spmi)
{
	...
	u8 alarm_flag = 0;	// add by xcb
	u8 alarm_value[4] = {0}; // add by xcb
	...
	// add by xcb begin
	rc = qpnp_read_wrapper(rtc_dd, &alarm_flag, 0x88e, 1);
	if (rc)
		dev_err(rtc_dd->rtc_dev, "SPMI read value at 0x88e failed\n");

	//dev_err(rtc_dd->rtc_dev, " * * * * * XCB * * * * * alarm_flag = 0x%x\n", alarm_flag);
	...
	alarm_flag = 0xFF;
	rc = qpnp_write_wrapper(rtc_dd, &alarm_flag, 0x88e, 1);
	if (rc)
		dev_err(rtc_dd->rtc_dev, "SPMI write 0xFF to 0x88e failed\n");
	// add by xcb end
...
fail_alarm_disable:
	...
	//dev_err(rtc_dd->rtc_dev, " * * * * * XCB * * * * * reg = 0x%x\n", reg);
...
/*
	if (charger_boot_mode) {
		...
		rc = qpnp_write_wrapper(rtc_dd, &reg,
			rtc_dd->alarm_base + REG_OFFSET_ALARM_CTRL1, 1);
		if (rc) {
			dev_err(rtc_dd->rtc_dev, "Write to ALARM ctrl reg failed\n");
		}
	}
*/
	if ((reg & BIT_RTC_ALARM_ENABLE) || charger_boot_mode) {
		/* set power off alarm flag 0xAA */
		reg = 0xAA;
		rc = qpnp_write_wrapper(rtc_dd, &reg, 0x88e, 1);
		if (rc)
			dev_err(rtc_dd->rtc_dev, "SPMI write 0xAA to 0x88e failed\n");
	} else {
		/* Clear Alarm register */
		rc = qpnp_write_wrapper(rtc_dd, value,
				rtc_dd->alarm_base + REG_OFFSET_ALARM_RW,
