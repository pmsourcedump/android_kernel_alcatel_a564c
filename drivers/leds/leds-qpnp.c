...
#define WLED_DRV_MIN_LEVEL 1
#define WLED_DRV_MAX_BRI 16
#define WLED_CURR_MAX_LEVEL 16
#define WLED_DRV_MIN_BRI 1
...
/*add by xiaopu.zhu for soul35evdo atc led*/
#define RGB_ATC_LED_ENABLE			0x4D
/*add by xiaopu.zhu for soul35evdo atc led*/
...
enum qpnp_leds {
	...
	QPNP_ATC_LED, // add by xiaopu.zhu for atc led
#endif
	QPNP_ID_MAX,
};
...
// add by qiang.zhaofor power off alarm bug id 610509
...
static struct qpnp_led_data *led_devp=NULL; //legen qin 20140423
...
#define BRIGHTNESS_MAX_SIZE 31 // Modified by Xiaodong.Chen at 2014.1.24 for change brightness max level to 31
extern bool panel_on_flag; //baoqiang add to fix backlight blink issue
...
static int qpnp_gpio_set_one_wire(struct qpnp_led_data *led)
{
	...
	if(panel_on_flag == true){ //baoqiang try to fix backlight blink when resume issue.
		panel_on_flag=false;
		msleep(150);//150 //modify by keys 20140306 for test
	}
...
#endif
#ifdef CONFIG_PROJECT_SOUL35EVDO
/*add by xiaopu.zhu for soul35evdo backlight*/
static int bl_level_l = WLED_DRV_MIN_BRI; 
static int resume = 0;
static int current_bl_level=1; 
static int current_bl_level_1=1;
static int qpnp_gpio_set_one_wire_aw9666(struct qpnp_led_data *led)
{
	unsigned int times=0;
    unsigned int i;
	unsigned long flags;
	int tl, th;
	int bl_level=WLED_DRV_MIN_BRI;

	if(panel_on_flag == true){ //baoqiang try to fix backlight blink when resume issue.
		panel_on_flag=false;
		msleep(300);
	}
	//pr_warn("%s, old backlight level = %d ---\n", __func__ ,bl_level_l);
	bl_level = (int)((led->cdev.brightness * WLED_DRV_MAX_BRI / led->cdev.max_brightness)+1);
	//pr_warn("%s, set backlight level = %d ---\n", __func__ ,bl_level);
	if (bl_level >=WLED_DRV_MAX_BRI) {
		bl_level = WLED_DRV_MAX_BRI;
	}
	if (bl_level <=WLED_DRV_MIN_BRI) {
		bl_level = WLED_DRV_MIN_BRI;
	}
	//pr_warn("%s, brightness: %d, level:%d\n", __func__, led->cdev.brightness, bl_level);
	
	local_irq_save(flags);
	if ((bl_level_l <= 0) && (resume == 1)){
		//pr_warn("%s, backlight on ,current backlight level = %d ---\n", __func__ ,bl_level); //baoqiang add to fix led bug 0211//modify by yusen.ke.sz@tcl.com at20140401
	...
	if (led->cdev.brightness<=0) 
		{
		//pr_warn("%s, backlight off,Before backlight level = %d ---\n", __func__, current_bl_level); //baoqiang.qin add to fix leds bug 0211//modify by yusen.ke.sz@tcl.com at20140401
		gpio_direction_output(led->gpio_cfg->gpio_num, 0);
		mdelay(3);
		bl_level_l = 0; // reset static variable 'bl_level_l'
		resume =1;
	...
	current_bl_level=WLED_CURR_MAX_LEVEL-bl_level;
	current_bl_level_1=WLED_CURR_MAX_LEVEL-bl_level_l;
	
	if (current_bl_level_1 > current_bl_level)
		{
			times = current_bl_level + WLED_DRV_MAX_BRI - current_bl_level_1;
		}
	else if (current_bl_level_1 < current_bl_level)
		{
				times = current_bl_level - current_bl_level_1;
		}
...
#ifdef CONFIG_PROJECT_SOUL35EVDO  
qpnp_gpio_set_one_wire_aw9666(led);//add by xiaopu.zhu
#else
...
#ifdef CONFIG_PROJECT_SOUL35EVDO
static int qpnp_atc_led_set(struct qpnp_led_data *led)
{
	if(led->cdev.brightness>0)
		qpnp_led_masked_write(led,led->base+RGB_ATC_LED_ENABLE,1, 1);
	else
		qpnp_led_masked_write(led,led->base+RGB_ATC_LED_ENABLE,1, 0);
				...
				enum led_brightness value)
{
	...
	if(bkl_flag==1)
		...
		if (rc < 0)
			dev_err(&led->spmi_dev->dev,
				"GPIO set brightness failed (%d)\n", rc);
		//Express Wire Protocol between the two have at least 1.5ms pulse interval
		udelay(1500);//add by yusen.ke.sz@tcl.com at 20140514 for 1.5ms pulse interval
#endif

		break;
#ifdef CONFIG_PROJECT_SOUL35EVDO
	case QPNP_ATC_LED:
		rc = qpnp_atc_led_set(led);
		break;
#endif
	default:
...
static int __devinit qpnp_led_set_max_brightness(struct qpnp_led_data *led)
{
	...
	case QPNP_ID_GPIO:  // add by shicuiping for gpio control led
#ifdef CONFIG_PROJECT_SOUL35EVDO
	case QPNP_ATC_LED:  // add by xiaopu.zhu for atc led
#endif
...
//add by baoqiang.qin
int qpnp_set_backlight_en(int value)
{
	if(!led_devp){
		pr_err("Can't get led data!\n");
		return -EINVAL;
	}
	if(value==255)
		value=102;
	led_devp->cdev.brightness=value;
#ifdef CONFIG_PROJECT_SOUL35EVDO
	qpnp_gpio_set_one_wire_aw9666(led_devp);
#else
	qpnp_gpio_set_one_wire(led_devp);
#endif
	pr_info("set backlight value: %d\n",value);

	return 0;
}

//add by baoqiang.qin end 
// add by shicuiping for gpio control led start
...
static int __devinit qpnp_leds_probe(struct spmi_device *spmi)
{
	...
	for_each_child_of_node(node, temp) {
...
#ifdef CONFIG_PROJECT_SOUL35EVDO
		} else if (strncmp(led_label, "atc", sizeof("atc")) == 0) {  // add by xiaopu.zhu for  atc led start
#endif
		} else {
		...
		if (led->default_on && !alarm_boot_mode) {	// modify by qiang.zhao for power off alarm bug id 610509
			led->cdev.brightness = 143;//adjust default level to 17, ignore "led->cdev.max_brightness";
			__qpnp_led_work(led, led->cdev.brightness);
			led_devp=led; //legen
			if (led->turn_off_delay_ms > 0)
				qpnp_led_turn_off(led);
		} else if(alarm_boot_mode)//add by yusen.ke.sz
		{

	      printk(KERN_ERR"led->id= %d\n",led->id);
	      printk(KERN_ERR"alarm_boot_mode= %d\n",alarm_boot_mode);
