...
static bool floated_charger_enable=true; // change by shicuiping for chg
...
static int msm_otg_set_power(struct usb_phy *phy, unsigned mA)
{
	 ...
	 */
	if (motg->chg_type == USB_SDP_CHARGER)
// [PLATFORM]-Add-BEGIN by TCTSZ.cuiping.shi, for car charger charge, 2014/06/04
	{
		if(mA>=100||mA<2)
			msm_otg_notify_charger(motg, mA);
		else if(mA<100)
			msm_otg_notify_charger(motg, 400);
	}
// [PLATFORM]-Add-END by TCTSZ.cuiping.shi, 2014/06/04
	return 0;
}
	...
	case USB_CHG_STATE_DCD_DONE:
		...
		if (vout && !dm_vlgc) { /* VDAT_REF < DM < VLGC */
			...
			if (line_state) /* DP > VLGC or/and DM > VLGC */
				...
				msm_otg_notify_charger(motg, IDEV_CHG_MIN);  // add by shicuiping for no usb D+ D- line chg
			}
			motg->chg_state = USB_CHG_STATE_DETECTED;
			delay = 0;
		}
		...
		if (test_bit(MHL, &motg->inputs)) {
			...
			case USB_CHG_STATE_DETECTED:
				...
				case USB_FLOATED_CHARGER:
					msm_otg_notify_charger(motg,
							IDEV_CHG_MIN); // change by shicuiping for chg
					pm_runtime_put_noidle(otg->phy->dev);
					pm_runtime_suspend(otg->phy->dev);
					break;
