...
// [PLATFORM]-Change by TCTSZ.cuiping.shi, for batt therm charge, 2014/04/10
enum qpnp_batt_therm_state {
	BATT_THERM_NORMAL = 0,
	BATT_THERM_COLD,
	BATT_THERM_WARM,
	BATT_THERM_HOT,
...
// [PLATFORM]-Change by TCTSZ.cuiping.shi, 2014/04/10
/**
...
// [PLATFORM]-Add-BEGIN by TCTSZ.cuiping.shi, for charging status change, 2014/04/09
#ifndef TARGET_BUILD_MMITEST
	int				soc_now;
#endif
// [PLATFORM]-Add-END by TCTSZ.cuiping.shi, 2014/04/09
...
// [PLATFORM]-Add-BEGIN by TCTSZ.cuiping.shi, for charging batt therm change, 2014/04/10
	bool				batt_init_ok;// [PLATFORM]-Add by TCTSZ.cuiping.shi, for cool batt died, 2014/06/06
	bool				batt_id_ok;
	enum qpnp_batt_therm_state	batt_therm_state_request;
	enum	qpnp_tm_state		batt_tm_status;
// [PLATFORM]-Add-END by TCTSZ.cuiping.shi, 2014/04/10
};
...
static enum power_supply_property msm_batt_power_props[] = {
	...
	POWER_SUPPLY_PROP_CHARGE_VOLTAGE, // add by shicuiping for get chg voltage
	POWER_SUPPLY_PROP_CHARGE_TEMP, // add by shicuiping for get chg temp
	POWER_SUPPLY_PROP_TEMP,
	POWER_SUPPLY_PROP_TEMP_VOL, //add by shicuiping for therm adc voltage
	POWER_SUPPLY_PROP_POWER_ON_VOLTAGE, //add by shicuiping for get power on voltage
...
// [PLATFORM]-Add-BEGIN by TCTSZ.cuiping.shi, for batt therm charge, 2014/04/10
	if (chip->batt_therm_state_request==BATT_THERM_HOT)
		return POWER_SUPPLY_HEALTH_OVERHEAT;
	else if(chip->batt_therm_state_request==BATT_THERM_COLD)
...
// [PLATFORM]-Add_BEGIN by TCTSZ.cuiping.shi, for cap disp, 2014/05/20
		if(ret.intval<0){
			pr_info("%s: get cap error!!!  value:%d",__func__,ret.intval);
			return DEFAULT_CAPACITY;
		}
// [PLATFORM]-Add_END by TCTSZ.cuiping.shi, 2014/05/20
		return ret.intval;
	}
	return DEFAULT_CAPACITY;
}
...
// [PLATFORM]-Add-BEGIN by TCTSZ.cuiping.shi, for charging status change, 2014/04/09
		...
		if (soc == 100 &&(chip->soc_now!=DEFAULT_CAPACITY)&&(chip->soc_now<100)
			...
			soc = chip->soc_now;
		else if(get_prop_batt_status(chip) == POWER_SUPPLY_STATUS_FULL  && (soc < 100))
			soc = 100;
		chip->soc_now=soc;
#endif
// [PLATFORM]-Add-END by TCTSZ.cuiping.shi, 2014/04/09
...
#ifndef TARGET_BUILD_MMITEST
	 chip->soc_now=DEFAULT_CAPACITY;// [PLATFORM]-Add by TCTSZ.cuiping.shi, for charging status change, 2014/04/09
#endif
	return DEFAULT_CAPACITY;
}
...
#define BATT_HOT_HOLD 550
#define BATT_WARM_HOLD 450
#define BATT_COLD_HOLD 0
#define BATT_BACK_HOLD 20
...
// [PLATFORM]-Add_BEGIN by TCTSZ.cuiping.shi, for batt therm charge, 2014/04/10
#if QPNP_TEMP_CAN_WRITE
	if( (temp_init>(-500)) && (temp_init<1300))
		results.physical  = temp_init;
#endif

	if(chip->batt_init_ok){// [PLATFORM]-Add by TCTSZ.cuiping.shi, for cool batt died, 2014/06/06
	if(results.physical > chip->adc_param.high_temp )
		chip->batt_tm_status =ADC_TM_WARM_STATE;
	else if(results.physical < chip->adc_param.low_temp )
		chip->batt_tm_status =ADC_TM_COOL_STATE;
	else
		chip->batt_tm_status =ADC_TM_STATE_NUM;

	if(chip->batt_tm_status == ADC_TM_WARM_STATE)
	{
		switch (chip->batt_therm_state_request) {
		case BATT_THERM_NORMAL:
			chip->batt_therm_state_request= BATT_THERM_WARM;
			chip->adc_param.high_temp = BATT_HOT_HOLD;
			chip->adc_param.low_temp = BATT_WARM_HOLD-BATT_BACK_HOLD;
			if(chip->batt_id_ok){
		...
		case BATT_THERM_WARM:
			chip->batt_therm_state_request= BATT_THERM_HOT;
			chip->adc_param.high_temp = 601;
			chip->adc_param.low_temp = BATT_HOT_HOLD-50;
			if(chip->batt_id_ok){
				//qpnp_chg_buck_control(chip, 0);
				chip->charging_disabled=true;
				qpnp_chg_charge_en(chip,!chip->charging_disabled);
				schedule_delayed_work(&chip->eoc_work,
					msecs_to_jiffies(EOC_CHECK_PERIOD_MS/10));
			}
			break;
		case BATT_THERM_COLD:
			chip->batt_therm_state_request= BATT_THERM_NORMAL;
			chip->adc_param.high_temp = BATT_WARM_HOLD;
			chip->adc_param.low_temp = BATT_COLD_HOLD;
			if(chip->batt_id_ok){
	...
	else if(chip->batt_tm_status == ADC_TM_COOL_STATE)
	{
		switch (chip->batt_therm_state_request) {
		case BATT_THERM_NORMAL:
			chip->batt_therm_state_request= BATT_THERM_COLD;
			chip->adc_param.high_temp = BATT_COLD_HOLD+BATT_BACK_HOLD;
			chip->adc_param.low_temp = -201;
			if(chip->batt_id_ok){
				//qpnp_chg_buck_control(chip, 0);
				chip->charging_disabled=true;
				qpnp_chg_charge_en(chip,!chip->charging_disabled);
	...
	}// [PLATFORM]-Add by TCTSZ.cuiping.shi, for cool batt died, 2014/06/06
// [PLATFORM]-Add_END by TCTSZ.cuiping.shi, for batt therm charge, 2014/04/10
...
static int get_prop_power_on_voltage(struct qpnp_chg_chip *chip) // get power on voltage
{
...
static int get_prop_charge_temp(struct qpnp_chg_chip *chip) // get chg temp
{
	int rc = 0;
	struct qpnp_vadc_result results;
...
static int get_prop_charge_voltage(struct qpnp_chg_chip *chip) // get chg voltage
{
	int rc = 0;
	struct qpnp_vadc_result results;
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_POWER_ON_VOLTAGE: // for get power on voltage
		...
		break;
	case POWER_SUPPLY_PROP_CHARGE_VOLTAGE: // for get chg voltage
		val->intval = get_prop_charge_voltage(chip);
		break;
	case POWER_SUPPLY_PROP_CHARGE_TEMP:  // for get chg temp
...
// add by shicuiping for when batt id is error ,disable chg
#if 1
/*Add by xiaopu.zhu for soul35EVDO charger*/
#ifdef CONFIG_PROJECT_SOUL35EVDO
		if((result.physical>100000) && (result.physical<200000))//BYD 1300
#else
/*Add by xiaopu.zhu for soul35EVDO charger*/
		if( ((result.physical>200000) && (result.physical<400000)) // BYD batt
			||((result.physical>1100000) && (result.physical<1300000)))// SCUD batt
#endif
		{
			chip->charging_disabled = false;
			chip->batt_id_ok=true;
		}else{
			chip->charging_disabled = true;
			chip->batt_id_ok=false;
...
#ifdef TARGET_BUILD_MMITEST // [PLATFORM]-Add by TCTSZ.cuiping.shi, for charging status change, 2014/03/31
...
qpnp_charger_probe(struct spmi_device *spmi)
{
	...
	schedule_delayed_work(&chip->aicl_check_work,
		msecs_to_jiffies(EOC_CHECK_PERIOD_MS));
	chip->batt_init_ok=true;// [PLATFORM]-Add by TCTSZ.cuiping.shi, for cool batt died, 2014/06/06
	pr_err("success chg_dis = %d, bpd = %d, usb = %d, dc = %d b_health = %d batt_present = %d\n",
			chip->charging_disabled,
			chip->bpd_detection,
			qpnp_chg_is_usb_chg_plugged_in(chip),
			qpnp_chg_is_dc_chg_plugged_in(chip),
