...
#define YARIS5TMO_TP		0x01
#define YARIS5NA_TP		0x02

//add by rongxiao.deng for TP compatible
#define PANEL_MINX		0 
#define PANEL_MINY		0 
#define X_MIN			0
#define Y_MIN			0
#ifdef CONFIG_PROJECT_YARIS5TMO
#define HARDWARE_TYPE_SYSTEM	YARIS5TMO_TP	
#define PANEL_MAXX		540 
#define PANEL_MAXY		960 
#define X_MAX			540		
#define Y_MAX			960	
#define DISP_MAXX		540
#define DISP_MAXY		960
#define PAN_MAXX		541
#define PAN_MAXY		1060
#endif

#ifdef CONFIG_PROJECT_YARIS5NA
#define HARDWARE_TYPE_SYSTEM	YARIS5NA_TP	
#define PANEL_MAXX		480 
#define PANEL_MAXY		854 
#define X_MAX			480		
#define Y_MAX			854	
#define DISP_MAXX		480
#define DISP_MAXY		854	
#define PAN_MAXX		481	
#define PAN_MAXY		1060	
#endif
//add end
...
//modify by junfeng.zhou.sz@tcl.com for delete the tmd2771x with 625727 begin . 20140319
//extern  int call_flag;
//modify by junfeng.zhou.sz@tcl.com for delete the tmd2771x with 625727 end . 
...
static irqreturn_t ft5x06_ts_interrupt(int irq, void *dev_id)
{
	...
	for (i = 0; i < data->pdata->num_max_touches; i++) {
		...
		if (status == FT_TOUCH_DOWN || status == FT_TOUCH_CONTACT) {
			//pressure = FT_PRESS;//delete by rongxiao.deng for touchpanel useless sometimes
			input_mt_report_slot_state(ip_dev, MT_TOOL_FINGER, 1);
			input_report_abs(ip_dev, ABS_MT_POSITION_X, x);
			input_report_abs(ip_dev, ABS_MT_POSITION_Y, y);
			//printk("hello hugo , x=%d, y=%d\n", x, y);
...
int fts_ctpm_auto_upg(struct i2c_client *client)
{
	...
	unsigned char version_list_yaris5tmo[] = {0x00, 0x10,0x11};  //Keep the yaris5tmo old version list that allows to be updated.
	unsigned char version_list_yaris5na[] = {0x00, 0x60, 0x61};  //Keep the yaris5na old version list that allows to be updated.
	int i,i_ret = -1;
	...
	printk("~~~~~~~~~~~~~~~~`uc_tp_fm_ver:%x,uc_host_fm_ver:%x\n", uc_tp_fm_ver, uc_host_fm_ver);

	if(hardware_type_tp == HARDWARE_TYPE_SYSTEM)//upgrade the tp thtches with the hardware to avoid upgrading by mistake
	{
    	switch(HARDWARE_TYPE_SYSTEM){
			case YARIS5TMO_TP:
				for (i = 0; i < sizeof(version_list_yaris5tmo)/sizeof(version_list_yaris5tmo[0]); i++)
				{
					if (uc_tp_fm_ver == version_list_yaris5tmo[i])
					{
				...
				break;
			case YARIS5NA_TP:
				printk("[FTS] The tp hardware type(0x%x) and the system hardware type(0x%x) both are yaris5na.\n",hardware_type_tp,HARDWARE_TYPE_SYSTEM);
				for (i = 0; i < sizeof(version_list_yaris5na)/sizeof(version_list_yaris5na[0]); i++)
				{
					if (uc_tp_fm_ver == version_list_yaris5na[i])
					{
						i_ret = fts_ctpm_fw_upgrade_with_i_file(client);
						if (i_ret == 0)
						{
		...
		printk("[FTS] ERROR!!! The tp hardware type (0x%x) doesn't match with the system hardware type(0x%x).\n",hardware_type_tp,HARDWARE_TYPE_SYSTEM);
...
static DEVICE_ATTR(focaltech_ver, 0664, ft5x06_fw_ver_show, ft5x06_fw_ver_store);
			...
			struct ft5x06_ts_platform_data *pdata)
{
	...
	pdata->panel_minx = PANEL_MINX;
	pdata->panel_miny = PANEL_MINY;
	pdata->panel_maxx = PANEL_MAXX;
	pdata->panel_maxy = PANEL_MAXY;
...
*/
	...
	pdata->disp_maxx = DISP_MAXX;
	pdata->disp_maxy = DISP_MAXY;
	pdata->pan_maxx = PAN_MAXX;
	pdata->pan_maxy = PAN_MAXY;
			   ...
			   const struct i2c_device_id *id)
{
 ...
 if(err)
	...
	dev_info(&client->dev, "HW ID = 0x%x(1:TMO,2:NA)\n", reg_value);
	if(0 == reg_value)
	{
		printk("!!!!!!The firmware was destroied, check what kind of Project, and then force recover the firmware\n");
		ret = fts_ctpm_update_project_setting(client);
		if(0x30 == ret )
			 hardware_type_tp = YARIS5TMO_TP;
		else if(0x31 == ret )
			 hardware_type_tp = YARIS5NA_TP;
	}
	else if ( reg_value <= 2 )
	{
		hardware_type_tp = reg_value;
	}

#if defined (CONFIG_TOUCHSCREEN_FT5X06_FIRMWARE)

	err=fts_ctpm_fw_i_file_config(client);
	if(err<0)
	{
