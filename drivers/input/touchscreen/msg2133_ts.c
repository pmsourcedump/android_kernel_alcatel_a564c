...
{
	u32 update_bin_major = 0;
	u32 update_bin_minor = 0;
	...
	if((update_bin_major== curr_ic_major && update_bin_minor> curr_ic_minor) || curr_ic_major == 0xffff)
	{
		u32 i,j;
		//printk("jliuzhiyu msg2133a_auto_update \n");
		for (i = 0; i < 33; i++)
		{
			for (j = 0; j < 1024; j++)
			{
				temp[i][j] = MSG21XX_update_bin[i*1024+j];
			}
		}
