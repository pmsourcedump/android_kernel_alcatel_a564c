...
//add by rongxiao.deng for adding power regulator
#ifdef POWER_REGULATOR
...
{
#if 0//delete by rongxiao.deng for bug 673388
...
//add by rongxiao.deng for adding power regulator
#ifdef POWER_REGULATOR
		ret = bma2x2_power_on(bma222_data,true);
		if(ret)
...
#endif
#ifdef POWER_REGULATOR
		ret = bma2x2_power_on(bma222_data,false);
		if(ret)
...
*/
	...
	bsd.y = val->y;
	bsd.z = val->z;
	bst_remap_sensor_data_dft_tab(&bsd,0);
/*
...
static void bma2x2_set_enable(struct device *dev, int enable)
{
	...
	if (enable) {
		if (pre_enable == 0) {
#ifdef POWER_REGULATOR
		ret = bma2x2_power_on(bma2x2,true);
		if(ret)
			printk("power on failed\n");
#endif
            if(bma2x2->in_suspend_f == 0){
		...
		if (pre_enable == 1 && bma2x2->in_suspend_f == 0) {
			bma2x2_set_mode(bma2x2->bma2x2_client,
					BMA2X2_MODE_SUSPEND);
#ifndef CONFIG_BMA_ENABLE_NEWDATA_INT
...
#ifdef POWER_REGULATOR
		ret = bma2x2_power_on(bma2x2,false);
		if(ret)
			printk("power on failed\n");
#endif
		}
...
static int bma2x2_power_init(struct bma2x2_data *data, bool on)
{
	int rc;
        ...
        if (regulator_count_voltages(data->vdd) > 0) {
                rc = regulator_set_voltage(data->vdd, 2600000,
                                           3300000);
                if (rc) {
                        dev_err(&data->bma2x2_client->dev,
        ...
        if (regulator_count_voltages(data->vio) > 0) {
                rc = regulator_set_voltage(data->vio, 1800000,
                                           1800000);
                if (rc) {
                        dev_err(&data->bma2x2_client->dev,
...
pwr_deinit:
        if(NULL == data->vdd)
                printk("bma2x2_data->vdd is NULL\n");
...
#endif
	atomic_set(&data->delay, BMA2X2_MAX_DELAY/5);// modify by rongxiao.deng for raise data rate
