...
static int mmc3xxx_i2c_rx_data(char *buf, int len)
{
	...
	//modify(add) by junfeng.zhou.sz for add i2c lock with 666626 begin . 20140512
    if(mmc3416x_in_suspend == 1)
    {
        printk(KERN_ERR "%s: devices in suspend\n", __func__);
        return -EIO;     
    }
	//modify(add) by junfeng.zhou.sz for add i2c lock with 666626 end . 20140512
	//modify(add) by junfeng.zhou.sz for add i2c lock with 656597 begin . 20140421
    mutex_lock(&mmc3416_i2c_lock);
...
static long mmc3416x_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case MMC3416X_IOC_READXYZ:
	    //modify by junfeng.zhou.sz for fix the ecompass impact camera with 579585 begin . 20131225 
		...
		//modify(add) by junfeng.zhou.sz for fix the ecompass impact camera with 579585 end .
		/* wait TM done for coming data read */
		msleep(MMC3416X_DELAY_TM);
#if READMD
		/* Read MD */
		...
		vec[1] = data[3] << 8 | data[2];
		vec[2] = data[5] << 8 | data[4];
		//add by junfeng.zhou.sz for tune compass param with 580135 begin . 20131226
		vec[2] = 65536 - vec[2];	
		//add by junfeng.zhou.sz for tune compass param with 580135 end .
	#if DEBUG
		printk("[X - %04x] [Y - %04x] [Z - %04x]\n", 
			vec[0], vec[1], vec[2]);
	#endif
...
#if 0 //add by junfeng.zhou . cancel allocate polled_device.
...
#endif //add by junfeng.zhou . cancel allocate polled_device.
...
static int mmc3416x_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	this_client = client;
	//modify(add) by junfeng.zhou.sz for add i2c lock with 666626 begin . 20140512
    mmc3416x_in_suspend = 0;
	//modify(add) by junfeng.zhou.sz for add i2c lock with 666626 end . 20140512
	//modify(add) by junfeng.zhou.sz for add i2c lock with 656597 begin . 20140421
	mutex_init(&mmc3416_i2c_lock);
	//modify(add) by junfeng.zhou.sz for add i2c lock with 656597 end . 
	//add by junfeng.zhou.sz for fix the ecompass impact camera with 579585 begin . 20131225 
	//read product_id_1 Reset Value 0x06
	data[0] = MMC3416X_REG_PRODUCTID_1;
	if (mmc3xxx_i2c_rx_data(data, 1) < 0) {
	    pr_err("%s: mmc3416x_device register failed,not read the product_id_1 value\n", __FUNCTION__);
	    goto out;
	}
	if(data[0] != 0x06)
	{
	    pr_err("%s: the device is not fit product_id_1 = %x\n", __FUNCTION__,data[0]);
	    goto out;
	}
	//add by junfeng.zhou.sz for fix the ecompass impact camera with 579585 end . 
...
//	ipdev = mmc3416x_input_init(client);  //modify by junfeng.zhou . cancel allocate polled_device. 
	mutex_init(&lock);
	printk("mmc3416x probe exit\n");      //modify by junfeng.zhou . just for info.
	return 0;

out_deregister:
	printk("mmc3416x probe out_deregister\n");   //modify by junfeng.zhou . just for info.
	misc_deregister(&mmc3416x_device);
	//modify(add) by junfeng.zhou.sz for add i2c lock with 656597 begin . 20140421
	mutex_destroy(&mmc3416_i2c_lock);
	//modify(add) by junfeng.zhou.sz for add i2c lock with 656597 end . 
out:
	return res;
}
...
static int mmc3416x_remove(struct i2c_client *client)
{
	...
	mutex_destroy(&mmc3416_i2c_lock);
	//modify(add) by junfeng.zhou.sz for add i2c lock with 656597 end . 
	//if (ipdev) input_unregister_polled_device(ipdev);   //modify by junfeng.zhou . cancel allocate polled_device. 

	return 0;
}


static int mmc3416x_suspend(struct i2c_client *client, pm_message_t mesg)
{
	//struct akm8975_data *akm = i2c_get_clientdata(client);

	/* TO DO: might need more work after power mgmt
	   is enabled */
	mmc3416x_in_suspend = 1;
	return 0;
}
