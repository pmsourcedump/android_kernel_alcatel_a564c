...
static int64_t bat_voltage_now=0; //add by shicuiping for therm adc voltage
		...
		struct qpnp_vadc_result *adc_chan_result)
{
	...
	bat_voltage = qpnp_adc_scale_ratiometric_calib(adc_code,
			adc_properties, chan_properties);
	bat_voltage_now = bat_voltage; //add by shicuiping for therm adc voltage
...
int32_t qpnp_adc_scale_batt_therm_voltage(struct qpnp_vadc_chip *chip,
		int32_t adc_code,
		const struct qpnp_adc_properties *adc_properties,
		const struct qpnp_vadc_chan_properties *chan_properties,
		struct qpnp_vadc_result *adc_chan_result)
{
	//pr_info("::::%s,line:%d, bat_voltage:%lld\n",__func__,__LINE__,bat_voltage);

	adc_chan_result->physical = bat_voltage_now;
	return 0;
}

EXPORT_SYMBOL(qpnp_adc_scale_batt_therm_voltage);
