...
static void ov5648_write_i2c(struct msm_sensor_ctrl_t *s_ctrl, uint32_t addr, uint16_t data)
{
	...
	if(0 > rc)
		pr_err("%s, %d, i2c write(0x%x) fail:%ld\n", __func__, __LINE__, addr, rc);
}
...
static int ov5648_read_i2c(struct msm_sensor_ctrl_t *s_ctrl, uint32_t addr)
{
	...
	rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read(s_ctrl->sensor_i2c_client, \
		addr, &data, MSM_CAMERA_I2C_BYTE_DATA);
	if(0 > rc)
		pr_err("%s, %d, i2c read(0x%x) fail:%ld\n", __func__, __LINE__, addr, rc);
	...
	struct ov5648_otp_struct *current_otp)
{
	...
	if(NULL == current_otp) {
		pr_err("%s, %d, OTP struct point is NULL\n", __func__, __LINE__);
		return 0;
	}
	...
	if(rc) 
		pr_err("%s, %d, OTP WB group(%d), rg_ratio:%d, bg_ratio:%d\n", \
			__func__, __LINE__, nGroup, \
			current_otp->rg_ratio, current_otp->bg_ratio);
	else 
		pr_err("%s, %d, OTP WB group(%d) invalid or empty\n", __func__, __LINE__, nGroup);
	...
	void __user *argp)
{
	...
	if (NULL == s_ctrl) {
		pr_err("%s, %d, msm_sensor_ctrl_t struct point is NULL\n", __func__, __LINE__);
		return 0;
	}
	...
	if(3 > i) {
		...
		pr_err("%s, %d, OTP WB after calculate, G_gain:%d, B_gain:%d, R_gain:%d\n", __func__, __LINE__, \
