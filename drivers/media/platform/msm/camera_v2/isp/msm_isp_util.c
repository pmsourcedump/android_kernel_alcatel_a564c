...
//patch by qiangbo.ni for burst shot vertical line issue 2014-05-26 begin
...
irqreturn_t msm_isp_process_irq(int irq_num, void *data)
{
	...
	msm_isp_process_overflow_irq(vfe_dev,
		&irq_status0, &irq_status1);	//patch by qiangbo.ni for burst shot vertical line issue 2014-05-26
	vfe_dev->hw_info->vfe_ops.core_ops.
...
void msm_isp_do_tasklet(unsigned long data)
{
	...
	while (atomic_read(&vfe_dev->irq_cnt)) {
		...
		if (atomic_read(&vfe_dev->error_info.overflow_state) !=
			...
			continue;
		}	//patch by qiangbo.ni for burst shot vertical line issue 2014-05-26 
...
int msm_isp_open_node(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	rc = vfe_dev->hw_info->vfe_ops.core_ops.reset_hw(vfe_dev,
		ISP_RST_HARD, 1);	//patch by qiangbo.ni for burst shot vertical line issue 2014-05-26 
	if (rc <= 0) {
...
int msm_isp_close_node(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	rc = vfe_dev->hw_info->vfe_ops.axi_ops.halt(vfe_dev, 1);	//patch by qiangbo.ni for burst shot vertical line issue 2014-05-26
	if (rc <= 0)
