...
extern void fbcon_setup(u32 format);//add by jch for imei number check
extern void fb_update_parm(u32 width, u32 height,u32 stride);//add by jch for imei number check
...
static int mdss_fb_register(struct msm_fb_data_type *mfd)
{
	...
	fbcon_setup(mfd->fb_imgType);
	fb_update_parm( fbi->var.xres, fbi->var.yres,fix->line_length);
