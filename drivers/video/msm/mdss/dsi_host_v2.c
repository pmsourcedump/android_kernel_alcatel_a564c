...
#define SOUL35_ESD_CHECK

extern u32 mipi_d2l_read_reg(struct mdss_dsi_ctrl_pdata *ctrl, u16 reg);
extern int mipi_d2l_write_reg(struct mdss_dsi_ctrl_pdata *ctrl, u16 reg, u32 data);
#endif
...
void msm_dsi_host_init(struct mipi_panel_info *pinfo)
{
	...
	MIPI_OUTP(ctrl_base + DSI_ERR_INT_MASK0, 0x13ff3ff0);// cwy disable  INT_MASK0
...
#ifdef SOUL35_ESD_CHECK
int ESD_EHECK_BL_SOUL35=0;
int start_check_esd=0;
...
if(start_check_esd==20)
{
	 dsi_set_tx_power_mode(0);
	 panel_status = mipi_d2l_read_reg(ctrl_pdata, 0x09);
	 dsi_set_tx_power_mode(1);
	 panel_status = ntohl(panel_status);	 
	// printk(KERN_ERR"SOUL35 LCD ID@@@@ is 0x%x\n",panel_status);
	 if (panel_status != 0xa4730600)
	 	{
	 	printk(KERN_ERR"SOUL35 LCD status register is 0x%x\n",panel_status);
	 	printk(KERN_ERR"The LCD work abnormal ESD recovery\n");
...
#else
/*cwy add yaris5na LCD esd check  */
static char panel_cmds[2]={0x00,0x00};
static struct dsi_cmd_desc panel_status_cmd = {
	{DTYPE_DCS_READ, 1, 0, 1, 0, sizeof(panel_cmds)},
	panel_cmds,
};
static void msm_dsi_read_panel_regs(struct mdss_dsi_ctrl_pdata *ctrl, char cmd, int len)
{
	...
	panel_cmds[0]=cmd;
	memset(rp->data,0,6);
	if(ctrl->panel_esd_data.esd_panel_name==HX8379A_PANEL){
		msm_dsi_cmds_rx(ctrl, &panel_status_cmd, len);
		msm_dsi_cmds_rx(ctrl, &panel_status_cmd, len);
	}else{
		msm_dsi_cmds_rx_2(ctrl, &panel_status_cmd, len);
		msm_dsi_cmds_rx_2(ctrl, &panel_status_cmd, len);
	}
}
...
bool msm_dsi_check_panel_status(struct mdss_dsi_ctrl_pdata *ctrl)
{
	char rbuf[6]={0x00}; 
	//printk("esd_panel_name %d\n",ctrl->panel_esd_data.esd_panel_name);
	if(ctrl->panel_esd_data.esd_panel_name==NONE_PANEL)
		return true;
	if(ctrl->panel_esd_data.esd_panel_name==HX8379A_PANEL){
		msm_dsi_read_panel_regs(ctrl,0x0A,5);
		rbuf[0]=ctrl->rx_buf.data[2];
		msm_dsi_read_panel_regs(ctrl,0x0C,5);
		rbuf[1]=ctrl->rx_buf.data[2];
		msm_dsi_read_panel_regs(ctrl,0x0D,5);
		rbuf[2]=ctrl->rx_buf.data[2];
		msm_dsi_read_panel_regs(ctrl,0x0E,5);
		rbuf[3]=ctrl->rx_buf.data[2];
		//pr_err("panel error status, regs 0A,0C,0D,0E: %x,%x,%x, %x",rbuf[0],rbuf[1],rbuf[2],rbuf[3]);
		ESD_EHECK_BL = 1;

		if(rbuf[0]!=0x1c || rbuf[1]!=0x70 || rbuf[2]!=0x00 || rbuf[3]!=0x81){
			ESD_EHECK_BL = 0;
			pr_err("panel error status, regs 0A,0C,0D,0E: %x,%x,%x, %x",rbuf[0],rbuf[1],rbuf[2],rbuf[3]);
			return false;
		}
	}else{
			msm_dsi_read_panel_regs(ctrl, 0x09, 1);
			rbuf[5]=ctrl->rx_buf.data[0];
			msm_dsi_read_panel_regs(ctrl, 0x0a, 1);
			rbuf[4]=ctrl->rx_buf.data[0];
			ESD_EHECK_BL = 1;
			//pr_err("panel error status reg_0A=0x%x reg_09=0x%x\n",rbuf[4],rbuf[5]);
			
			if(rbuf[4]!=0x9C || rbuf[5]!=0x80 ){
				ESD_EHECK_BL = 0;
				pr_err("panel error status reg_0A=0x%x reg_09=0x%x\n",rbuf[4],rbuf[5]);
				return false;
...
static int panel_esd_on= 0;
static int panel_esd_num= 0;
...
int msm_dsi_bta_status_check(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (panel_esd_on > 10)
	{
		...
		dsi_set_tx_power_mode(0);
		ret=msm_dsi_check_panel_status(ctrl_pdata);
		dsi_set_tx_power_mode(1);
	}
	else 
	{
	...
	if (ret <= 0)
		pr_err("%s: DSI BTA error: %i %d\n", __func__, __LINE__,panel_esd_num++);

	pr_debug("%s: BTA done with ret: %d %d \n", __func__, ret,panel_esd_num);
	return ret;
}


/*cwy add yaris5na LCD esd check end */
