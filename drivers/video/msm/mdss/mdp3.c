...
extern void fb_update_base(void *base);//add by jch for imei number check
u32 splash_addr=0;//legen
		...
		int client)
{
	...
	void *base;//add by jch for imei number check
	struct ion_client *iclient = mdp3_res->ion_client;
	int dom;
...
//add by jch for imei number check		
		base = ion_map_kernel(iclient,data->srcp_ihdl);		
		fb_update_base((unsigned char *)base);
...
int mdp3_parse_dt_splash(struct msm_fb_data_type *mfd)
{
	...
	splash_addr=mdp3_res->splash_mem_addr;//legen
